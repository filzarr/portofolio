...
export default function Navbar() {
    ...
    return (
            <>            <nav className={isScrolled ? "backdrop-blur-sm shadow-md underline-offset-8 dark:bg-drk fixed w-full max-sm:w-screen z-20 top-0 left-0 after:border-b after:shadow-sm border-gray-200 dark:border-gray-600 duration-75  " : " duration-75 py-3 underline-offset-8 dark:bg-drk fixed max-sm:w-screen md:w-full z-20 top-0 left-0 after:border-b after:shadow-sm border-gray-200 dark:border-gray-600"}>

                <div className="flex justify-between w-full  h-20 items-center px-3 md:px-16">
                    <div className="">
                        <p className="text-emerald-400 font-bold  md:text-2xl md:block hidden ">Filza Rizki Ramadhan</p>
                ...
                <div className={mobile ? " fixed top-0 left-0 right-0 bottom-0  overflow-x-hidden overflow-y-auto  z-30  w-full h-screen duration-75    p-4 o transition-transform  bg-white  dark:bg-gray-800" : "hidden"} >
                   <button onClick={() => isMobile(false)} data-drawer-hide="drawer-example" aria-controls="drawer-example" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 relative top-2.5 right-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                       <svg className="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                           <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                       </svg>
                       <span className="sr-only">Close menu</span>
                   </button>
                   <div className="py-4 relative   z-50  flex h-full w-full text-center justify-center items-center">
