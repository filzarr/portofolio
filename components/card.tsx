...
export  function Card({ img, judul, ket, stack }: AppCard) {
    ...
    return (
        <div className=" dark:border-gray-900/50 border dark:bg-[#132347] border-gray-100 shadow-md dark:shadow-gray-800/50 shadow-gray-300 p-5 rounded-md hover:shadow-2xl  duration-300">
            ...
            <div className="flex gap-5 mt-3">
                {stack.map((val, index) => (
                    <div className=" bg-gray-300 px-2 py-1 dark:bg-drk dark:border-gray-600 dark:border dark:text-gray-300" key={index}  >
...
export function Cardct({ img, judul, nohp, ket }: AppContact) {
    return (
        <div className=" dark:bg-[#132347] dark:shadow-black/30 dark:border-black/30 dark:border-b-emerald-400 group w-full flex border-b-4 rounded-b-xl rounded-md px-3 py-1  border border-gray-100  border-b-emerald-400  duration-100 shadow-xl  hover:shadow-gray-300">
            <div className=" flex min-h-full items-center">

                <Image className=" group-hover:-translate-y-3 duration-75" src={img} height={50} width={50} alt='Pict' />
            </div>
            <div className=" px-5 py-3">
                <p className='text-xl font-semibold text-gray-900 dark:text-white'>{judul}</p>
                <p className='text-sm mt-2  text-gray-600 font-medium dark:text-gray-300 '>{nohp}</p>
                <p className='font-semibold mt-2 text-md text-emerald-400'>{ket}</p>
            </div>
        </div>
    )
}
